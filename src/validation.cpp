...
#if defined(NDEBUG)
#error "KTV cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uktv) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k KTV
    ...
    } else if (nHeight <= nLast && nHeight >= 0) {
...
{
    util::ThreadRename("ktv-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Ktv Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nKtvBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nKtvBadBlockBits)) {
            // accept KTV block minted with incorrect proof of work threshold
            return true;
        }
                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zktvActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZktvActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZktvActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zktvActive || publicZktvActive)) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZKTVModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            ...
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zktv");
            }
        }
