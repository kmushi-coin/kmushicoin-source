...
const char * const KTV_CONF_FILENAME = "ktv.conf";
const char * const KTV_PID_FILENAME = "ktv.pid";
const char * const KTV_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ktvd -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\KTV
// Windows >= Vista: C:\Users\Username\AppData\Roaming\KTV
// Mac: ~/Library/Application Support/KTV
// Unix: ~/.ktv
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KTV";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KTVParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KTVParams
    // Mac: ~/Library/Application Support/KTVParams
    // Unix: ~/.ktv-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KTVParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "KTVParams";
#else
    // Unix
    return pathRet / ".ktv-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/ktv/sapling-spend.params";
        sapling_output = "/usr/share/ktv/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/ktv/sapling-spend.params";
            sapling_output = "/usr/local/share/ktv/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", KTV_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", KTV_PID_FILENAME));
