...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select KTV Outputs to Spend" :
            "Select Shielded KTV to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::KTV;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show KTV address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
