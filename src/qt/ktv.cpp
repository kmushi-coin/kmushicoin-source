...
#include "qt/ktv/splash.h"
#include "qt/ktv/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ktv-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ktv.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ktv.qrc)
...
/** Class encapsulating KTV startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (KTVGUI)
...
{
    window = new KTVGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &KTVGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &KTVGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(KTVGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(KTVGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // KTV: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KTVGUI::handlePaymentRequest);
        connect(window, &KTVGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. KTV can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data and blocks directory and parse ktv.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", KTV_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // ktv: links repeatedly have their payment requests routed to this process:
