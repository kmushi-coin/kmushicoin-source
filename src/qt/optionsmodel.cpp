...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::KTV);
    ...
    if (settings.contains("nAnonymizeKtvAmount") || reset)
        gArgs.SoftSetArg("-anonymizektvamount", settings.value("nAnonymizeKtvAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ktv.cpp during shotdown to also remove the window positions
