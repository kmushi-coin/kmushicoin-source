...
#include "qt/ktv/defaultdialog.h"
...
const QString KTVGUI::DEFAULT_WALLET = "~Default";

KTVGUI::KTVGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void KTVGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &KTVGUI::toggleHidden);
...
void KTVGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &KTVGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &KTVGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &KTVGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &KTVGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &KTVGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &KTVGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &KTVGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &KTVGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &KTVGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &KTVGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &KTVGUI::execDialog);
}


void KTVGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void KTVGUI::handleRestart(QStringList args)
{
...
void KTVGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &KTVGUI::message);
...
void KTVGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &KTVGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &KTVGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void KTVGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void KTVGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void KTVGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &KTVGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &KTVGUI::show);
                e->ignore();
            }
        }
    }
...
void KTVGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void KTVGUI::messageInfo(const QString& text)
{
...
void KTVGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool KTVGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void KTVGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void KTVGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KTVGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void KTVGUI::goToSend()
{
    showTop(sendWidget);
}

void KTVGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void KTVGUI::goToMasterNodes()
{
    showTop(masterNodesWidget);
}

void KTVGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void KTVGUI::goToSettings(){
    showTop(settingsWidget);
}

void KTVGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void KTVGUI::goToReceive()
{
    showTop(receiveWidget);
}

void KTVGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void KTVGUI::showTop(QWidget* view)
{
...
void KTVGUI::changeTheme(bool isLightTheme)
{
...
void KTVGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool KTVGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void KTVGUI::showHide(bool show)
{
...
int KTVGUI::getNavWidth()
{
    return this->navMenu->width();
}

void KTVGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
bool KTVGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &KTVGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &KTVGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &KTVGUI::message);
    connect(topBar, &TopBar::message, this, &KTVGUI::message);
    connect(sendWidget, &SendWidget::message,this, &KTVGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &KTVGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &KTVGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &KTVGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &KTVGUI::incomingTransaction);

    return true;
}

bool KTVGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void KTVGUI::removeAllWallets()
{
    // Single wallet supported.
}

void KTVGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(KTVGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KTVGUI::subscribeToCoreSignals()
{
...
void KTVGUI::unsubscribeFromCoreSignals()
{
