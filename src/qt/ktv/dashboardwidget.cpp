...
#include "qt/ktv/dashboardwidget.h"
#include "qt/ktv/forms/ui_dashboardwidget.h"
#include "qt/ktv/sendconfirmdialog.h"
#include "qt/ktv/txrow.h"
...
DashboardWidget::DashboardWidget(KTVGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareKtv, "square-chart-ktv");
    setCssProperty(ui->labelSquarezKtv, "square-chart-zktv");
    setCssProperty(ui->labelKtv, "text-chart-ktv");
    setCssProperty(ui->labelZktv, "text-chart-zktv");
    ...
    setCssProperty(ui->labelAmountKtv, "text-stake-ktv-disable");
    setCssProperty(ui->labelAmountZktv, "text-stake-zktv-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &KTVGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("KTV")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isKtv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZKTV;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair KTV, zKTV
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ktv = (pair.first != 0) ? pair.first / 100000000 : 0;
            zktv = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalKtv += pair.first;
            chartData->totalZktv += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesKtv.append(ktv);
        chartData->valueszKtv.append(zktv);

        int max = std::max(ktv, zktv);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesKtv);
    set1->append(chartData->valueszKtv);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalKtv > 0 || chartData->totalZktv > 0) {
        setCssProperty(ui->labelAmountKtv, "text-stake-ktv");
        setCssProperty(ui->labelAmountZktv, "text-stake-zktvxl");
    } else {
        setCssProperty(ui->labelAmountKtv, "text-stake-ktv-disable");
        setCssProperty(ui->labelAmountZktv, "text-stake-zktvxl-disable");
    }
    forceUpdateStyle({ui->labelAmountKtv, ui->labelAmountZktv});
    ui->labelAmountKtv->setText(GUIUtil::formatBalance(chartData->totalKtv, nDisplayUnit));
    ui->labelAmountZktv->setText(GUIUtil::formatBalance(chartData->totalZktv, nDisplayUnit, true));
