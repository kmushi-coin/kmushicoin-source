...
#include "qt/ktv/forms/ui_snackbar.h"
#include "qt/ktv/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(KTVGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &KTVGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
