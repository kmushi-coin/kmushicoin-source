...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZktv)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZktv)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZktv);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZktv)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZktv)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZktv);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter KTV address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no KTV: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::KTV, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert ktv:// to ktv:
    //
    //    Cannot handle this later, because ktv:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KTV, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", KTV_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "KTV (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "KTV (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "KTV.lnk";
}
...
{
    return GetAutostartDir() / "ktv.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a ktv.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=KTV (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=KTV (regtest)\n";
        else
            optionFile << "Name=KTV\n";
