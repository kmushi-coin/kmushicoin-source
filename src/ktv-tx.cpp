...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " ktv-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  ktv-tx [options] <hex-tx> [commands]  Update hex-encoded ktv transaction\n" +
                               "or:     ktv-tx [options] -create [commands]   Create hex-encoded ktv transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ktv transaction
