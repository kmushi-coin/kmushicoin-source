...
KTV v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
- `delegated_balance` - KTV balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - KTV balance held in cold staking addresses.
...
Arguments:
1. "stakingaddress"      (string, required) The ktv staking address to delegate.
2. "amount"              (numeric, required) The amount in KTV to delegate for staking. eg 100
3. "owneraddress"        (string, optional) The ktv address corresponding to the key that will be able to spend the stake.
...
Result:
amount              (numeric) The total amount in KTV received for this account in P2CS contracts.
```
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~ktv) repository, we are now offering a [Snap package](https://snapcraft.io/ktv-core) as quick way to install and update a KTV wallet.
...
- #936 `12a6b704b6` [zKTV] PublicCoinSpend v4 - Coin Randomness Schnorr Signature (random-zebra)
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for KTV v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for KTV v4.0 (random-zebra)
...
- #1125 `c6e238ca4d` [GUI] Inform if open ktv.conf and/or backups folder fail. (furszy)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong KTV website (NoobieDev12)
