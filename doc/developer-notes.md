...
KTV uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
KTV is a multi-threaded application, and deadlocks or other
...
**Sanitizers**
KTV can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as KTV, where everyone uses
...
pay attention to for reviewers of KTV code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: KTV uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against KTV so that they can be integrated
...
file descriptors it will cause problems with KTV's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:KTV-Project/KTV.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ktv-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ktv-cli` and the GUI debug console use this table to determine how to
