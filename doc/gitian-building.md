...
*Setup instructions for a gitian build of KTV using a VM or physical system.*

Gitian is the deterministic build process that is used to build the KTV
...
to the KTV GitHub release page.
...
- [Building KTV](#building-ktv-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ktv-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ktv-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ktv-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ktv-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ktv-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ktv-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ktv-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@kmushicoin.co) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ktv-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ktv-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ktv-osx-*-build.assert
```
