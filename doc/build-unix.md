UNIX BUILD NOTES
====================
Some notes on how to build KTV Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile KTV Core and the dependencies,
...
This will build ktv-qt as well, if the dependencies are met.
...
memory available when compiling KTV Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~ktv/+archive/ktv).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:ktv/ktv
    sudo apt-get update
...
To build KTV Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build ktv-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a ktv-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip ktvd" to strip the debug
...
Security
--------
To help make your KTV Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, KTV Core should be built with a non-executable stack
