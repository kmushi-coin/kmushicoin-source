...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/KTVParams"
    else
        PARAMS_DIR="$HOME/.ktv-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
KTV - install-params.sh

This script will install the KTV zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common KTV zkSNARK parameters. Note that it is
